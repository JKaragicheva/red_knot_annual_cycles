...
download.file("https://raw.githubusercontent.com/eldarrak/red_knot_annual_cycles/master/Example_bird_for_wavelet_cor.csv",
...
```{r, eval = F}
download.file("https://raw.githubusercontent.com/eldarrak/red_knot_annual_cycles/master/body_mass_dates.csv",
   "body_mass_dates.csv")
...
The obtained with CWT results were visually assessed for irrelevantly defined  characteristic points, and the latter were removed manually. Load the manually processed data
```{r, eval = F}
download.file("https://raw.githubusercontent.com/eldarrak/red_knot_annual_cycles/master/plumage_molt_dates.csv",
    ...
    if (nrow(plat)>0 & !is.na(onreal) & !is.na(off1real)){
        ...
        if ((off1real %in% example_data$DATE)){
            ...
            if (example_data[example_data$DATE==off1real,]$PLUM>base){
                ...
                #to the next date after the last date, when the molt score was larger than the baseline value
            } else {
                b<-rev(which(example_data$PLUM>base & example_data$DATE<=off1real))[1]+1
                onoffsets[onoffsets$Year==unique(onoffsets$Year)[i-1],]$offsetcor_2<-example_data$DATE[b]
            }
        }
        #if the CWT-defined onset and/or offset date do not exist, or the period between them is 0,
...
for(i in 1:length(unique(onoffsets$Year))){
    ...
    if (nrow(plat)>0 & !is.na(on)){
        ...
        #compare molt scores at the CWT-defined onsets and offsets to the full plumage score
        #in a corresponding year and set offsets of pre-alternate molt and 
...
```{r, eval = F}
download.file("https://raw.githubusercontent.com/eldarrak/red_knot_annual_cycles/master/wing_molt_dates.csv",
   "wing_molt_dates.csv")
